...
const Speak = (props: SpeakProps) => {
  ...
  const [sarathiPageContent, setsarathiPageContent] = useState({
    mainTitle:
    ...
    unHighlightedText: "or anything that matters to you!",
    timingText: "Available from 10am-1 pm & 4pm-7pm, all week",
  });
  useEffect(() => {
    get_data_from_firebase("sarathiSpeakPage").then((data: any) => {
      if (data) {
        setsarathiPageContent({ ...data });
      }
    });
  }, []);
  return (
    <React.Suspense fallback={<>Loading</>}>
      <MaxWidthWrapper>
        <div className="mb-10 p-5">
          <h1 className="text-3xl">Our Sarathis</h1>
          <p className="font-lightFont text-base">
            {sarathiPageContent.mainTitle}
            <span className="text-lavender">{sarathiPageContent.subText}</span>
            {sarathiPageContent.unHighlightedText}
          </p>
          <div className="flex flex-row justify-between items-center">
            <p className="text-lg font-normalFont">
              <ClockCircleOutlined /> {sarathiPageContent.timingText}
            </p>{" "}
        ...
        {isLoading ? (
          ...
          <div
            key={props.type}
            className={`flex flex-wrap gap-10 justify-center mb-8 p-2 pl-4`}
          >
            {sortedList?.map((saarthi, index) => (
              <React.Fragment key={`main-div-${index}-main`}>
                ...
                <div
                  key={`main-div-${index}`}
                  className="w-full xs:w-full sm:w-[50%] md:w-[40%] lg:w-[30%] xl:w-[30%] mb-5 gap-4"
                >
                  <SarathisCard
